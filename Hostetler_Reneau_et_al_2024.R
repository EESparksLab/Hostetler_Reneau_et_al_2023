#Hostetler et al., 2024 - SMURF: A Tool to Measure Maize Root System Stiffness That Enables a Comprehensive Understanding of Plant Mechanics and Lodging
library(agricolae)
citation("agricolae")
library(rcompanion)
citation("rcompanion")
...
data = read.csv("2021_RepeatTestingData.csv", header = TRUE, na.strings = "NA")
data$ID = paste(data$Plot, data$Replicate, sep="_")
data = subset(data, Test_Type != "D")
##Figure
plot.labs <- c("Plant 1", "Plant 2")
names(plot.labs) <- c("965_1", "965_2")
Fig3A = ggplot(data=data,aes(x=Test_Type, y=line_raw_slope_N.m, color=Additional_Notes))+
...
Fig3B =ggplot(data=data1,aes(x=Additional_Notes, y=line_raw_slope_N.m, color=Year))+
  geom_boxplot(size = 0.25, color="black") + 
...
data1 = subset(data, Genotype == "CML258")
head(data1)
year.labs <- c("2021", "2022")
names(year.labs) <- c("2021", "2022")
Fig3C =ggplot(data=data1,aes(x=Additional_Notes, y=line_raw_slope_N.m, color=Year))+
  geom_boxplot(size = 0.25, color="black") + 
  xlab("Time of Testing") + 
  ylab("Root System Stiffness (N/m)")+
  labs(color = "Days after Planting")+
...
Fig3C
ggdraw() +
  draw_plot(Fig3A, x = 0, y = 0, width = .33, height = 1) +
  draw_plot(Fig3B, x = 0.33, y = 0, width = .33, height = 1) +
  draw_plot(Fig3C, x = 0.66, y = 0, width = .33, height = 1) +
...
Fig4A=ggplot(data=data1,aes(x=Genotype, y=line_raw_slope_N.m))+
...
Fig4B=ggplot(data=data1,aes(x=Genotype, y=line_raw_slope_N.m))+
...
Fig4C=ggplot(data=data1,aes(x=Genotype, y=line_raw_slope_N.m))+
  geom_boxplot(fill="pink4")+
  xlab("Genotype") + 
  ylab("Root System Stiffness (N/m)")+
  labs(fill = "DAP")+
  scale_y_continuous(limits=c(0,12000), breaks=seq(0,12000,2000))+
...
Fig4D=ggplot(data=data1,aes(x=Genotype, y=line_raw_slope_N.m))+
...
Fig4E=ggplot(data=data1,aes(x=Genotype, y=line_raw_slope_N.m))+
...
Fig4F=ggplot(data2, aes(x=Hybrid, y = Rank, color = X)) +
  ...
  scale_color_manual(name = "Enviornmental Condition",
                    ...
                    values = c("mistyrose2", "slategray", "pink4", "cadetblue3", "indianred3"))+
  xlab("Genotype")+
...
Fig4F
ggdraw() +
  draw_plot(Fig4A, x = 0, y = 0.5, width = .33, height = 0.5) +
  draw_plot(Fig4B, x = 0.33, y = 0.5, width = .33, height = 0.5) +
  draw_plot(Fig4C, x = 0.66, y = 0.5, width = .33, height = 0.5) +
  draw_plot(Fig4D, x = 0, y = 0, width = .33, height = 0.5) +
  draw_plot(Fig4E, x = 0.33, y = 0, width = .33, height = 0.5) +
  draw_plot(Fig4F, x = 0.66, y = 0, width = .33, height = 0.5) +
