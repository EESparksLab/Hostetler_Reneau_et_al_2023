...
data2$Plant.Ht_mm = data2$Plant.Ht * 10
data2$ShootVolume = (pi*(((data2$StalkDiamAvg)/2)^2)*data2$Plant.Ht_mm)
...
data2$RTS_volume = (data2$Torsional_Stiffness_Nm.rad)/data2$ShootVolume
Fig6A=ggplot(data=data2,aes(x=Density, y=RTS_Ht, fill=Genotype.x))+
  geom_boxplot(size = 0.25) + 
  xlab("Density (plants/row)") + 
  ylab("RTS/Height")+
  labs(fill = "Density (plants/row)")+
  ...
  theme(legend.position = "none",
        ...
        strip.text.y = element_text(size = 12, colour = "black"))
Fig6A
Fig6B=ggplot(data=data2,aes(x=Density, y=RTS_SD, fill=Genotype.x))+
  geom_boxplot(size = 0.25) + 
  xlab("Density (plants/row)") + 
  ...
  theme(legend.position = "none",
        ...
        strip.text.y = element_text(size = 12, colour = "black"))
Fig6B
Fig6C=ggplot(data=data2,aes(x=Density, y=RTS_volume, fill=Genotype.x))+
  geom_boxplot(size = 0.25) + 
  xlab("Density (plants/row)") + 
  ylab("RTS/Shoot Volume")+
  labs(fill = "Genotype")+
  scale_fill_manual(values=c("pink4","slategray"))+
  theme(#legend.position = "none",
    axis.text.y = element_text(size=12), 
...
#Stats - Does Genotype or Density impact RTS_volume
attach(data2)
lm_x <- lm(RTS_volume ~ Genotype.x*Density)
anova(lm_x)
lm_x_aov=aov(lm_x) 
HSD.test(lm_x_aov, trt = c("Density","Genotype"), console = TRUE)
par(mfrow=c(2,2))
plot(lm_x)
par(mfrow=c(2,1))
plot(data2$RTS_volume)
hist(data2$RTS_volume)
resid = residuals(object = lm_x_aov)
shapiro.test(x=resid)
leveneTest(RTS_volume ~ Genotype.x*Density)
detach(data2)

FigS4=ggplot(data=data2,aes(x=Density, y=ShootVolume, fill=Genotype.x))+
  geom_boxplot(size = 0.25) + 
  xlab("Density (plants/row)") + 
  ylab("Shoot Volume (mm3)")+
  labs(fill = "Hybrid")+
  scale_fill_manual(values=c("pink4","slategray"))+
  theme(
    axis.text.y = element_text(size=12), 
...
#Stats - Does Genotype or Density impact Shoot Volume
attach(data2)
lm_x <- lm(ShootVolume ~ Genotype.x*Density)
anova(lm_x)
lm_x_aov=aov(lm_x) 
HSD.test(lm_x_aov, trt = c("Density","Genotype"), console = TRUE)
HSD.test(lm_x_aov, trt = c("Density"), console = TRUE)
...
plot(lm_x)
par(mfrow=c(2,1))
plot(data2$ShootVolume)
hist(data2$ShootVolume)
resid = residuals(object = lm_x_aov)
shapiro.test(x=resid)
leveneTest(ShootVolume ~ Genotype.x*Density)
detach(data2)
...
df$Genotype = factor(df$Genotype,levels = c("PHB47 x LH198","PHB47 x PHP02","LH82 x DK3IIH6","PHK56 x PHK76","PHK76 x PHP02",
                                            "LH145 x DK3IIH6","DK3IIH6 x LH198","LH198 x PHN46","LH82 x PHK76","LH82 x PHP02","LH82 x LH145"))
FigS2A=ggplot(data=df,aes(x=Genotype, y=percent_lodged, fill=Rep))+
...
FigS2B=ggplot(df, aes(Row, Column, fill= percent_lodged)) + 
  geom_tile()+
...
FigS2B
ggdraw() +
  draw_plot(FigS2B, x = 0, y = 0, width = 1, height = 0.5) +
  draw_plot(FigS2A, x = 0, y = 0.5, width = 1, height = 0.5) +
